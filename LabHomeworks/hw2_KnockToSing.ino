...
unsigned long int lastNoteMillis = 0;
...
void sing() {
  ...
  if (millis() - lastNoteMillis < pauseBetweenNotes) {
    return;
  }
...
void loop() {
  ...
  if (sensorReading >= threshold and !melodyOn and !waitingForMelody) {
    waitingForMelody = 1;
    lastMillis = millis();
  }
  if (waitingForMelody) {
    currentMillis = millis();
    if (currentMillis - lastMillis >= melodyDelay) {
      waitingForMelody = 0;
      melodyOn = 1;
    }
  }
